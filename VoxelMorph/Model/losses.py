...
def masked_mse_loss(x, y, mask):
    x = x * mask
    y = y * mask
    unmask_count = mask.sum().item() * x.shape[0]
    if unmask_count == 0:
        return 0
    return torch.sum((x - y) ** 2) / unmask_count


def mask_loss(mask):
    count = (torch.le(mask, 0.5)).sum().item()
    percent = count / mask.numel()
    return 1 / (1 + torch.exp(torch.tensor(-(percent * 30 - 30))))
...
def dice_oasis(pred1, truth1):
    dice_35 = []
    smooth = 1e-5
    masks = torch.unique(torch.cat((pred1, truth1), 0))
    for k in masks:
        if k == 0:
            continue
        truth = truth1 == k
        pred = pred1 == k
        intersection = torch.sum(pred[truth == 1.0]) * 2.0
        dice_35.append(intersection / (torch.sum(pred) + torch.sum(truth) + smooth))
    return torch.mean(torch.tensor(dice_35)) if dice_35 else torch.tensor(1)
...
def NJ_loss(ypred):
    ...
    '''
    Neg_Jac = 0.5 * (torch.abs(Jacobian(ypred)) - Jacobian(ypred))
